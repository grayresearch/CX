...
multiplexing type, (e.g., _off_, _type-1_, ...) and specify the
...
.`mcx_selector` CSR 0xBC0 (type 0: legacy custom instructions))
[wavedrom,target="`mcx_selector_0`",svg]
....
{reg:[
{bits: 28, name: 'reserved', type:1, vspace:88},
{bits: 4, name: 0, attr:'type'},
],
config: {lanes: 1, bits: 32}
}
....

.`mcx_selector` CSR 0xBC0 (type 1: extension multiplexing)
...
* When `type=0`, disable composable extension multiplexing. The rest
of `mcx_selector` is ignored. No CXU is selected. `Custom-[0123]`
instructions execute the CPU's built-in custom instructions.

* When `type=1`, enable _type 1_ composable extension multiplexing. The
...
[TIP]
====
The `type` field provides backwards compatibility with legacy
custom extensions, and forwards compatibility with future CX systems.
In future a new CX type may be added, with a new layout and interpretation
of selector fields and new means of decoding custom instruction fields
into CXU requests.
====
...
* Set by a CSR-write to `mcx_selector`, or by a CF instruction, when `mcx_selector.type` is invalid.
...
One option: fields and behavior determined by hart's current CX type (`mcx_selector.type`).
This becomes unwieldy when multiplexing between extensions switches different types.
