...
{bits: 10, name: 'state_id'},
{bits: 2, name: 'rsvd', type:1},
{bits: 4, name: 'mode'},
],
...
* When `mode=1`, enable *mode 1* composable extension multiplexing. The
...
[NOTE]
====
TBD: What must happen upon a CSR-write of an unsupported mode to
`mcx_selector.mode`? Set a new error flag in `cx_status`? Exception?
====
...
[NOTE]
====
How to best anticipate future changes to `cx_version`?
...
One option: add a `cx_version.type` field, selecting an interpretation of `cx_status` CSR fields.
