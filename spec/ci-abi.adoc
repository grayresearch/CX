...
supported, each hart has a `mcx_selector_table` CSR to address its CX
selector table.
...
shall implement the `mcx_selector_table` and `cx_selector_index` CSRs.
...
==== `mcx_selector_table` CSR 0xBC1: CX selector table base

When CX access control (§<<_cx_access_control>>) is supported, the
`MXLEN`-bit-wide `mcx_selector_table` CSR specifies the base address
of the hart's CX selector table. The CSR may be read and written in
machine level.

.mcx_selector_table CSR 0xBC1 (when MXLEN=32)
[wavedrom,target="cx_selector_table",svg]
....
{reg:[
{bits: 12, name: '0'},
...
CSR-writes to `mcx_selector_table` zero the twelve least significant
bits of the table address, so a CX selector table address must be 4
KB aligned.

==== `cx_selector_index` CSR 0x800: CX selector index

When CX access control (§<<_cx_access_control>>)  is supported, the
`cx_selector_index` CSR selects an entry from the hart's CX selector
table entry to write to the `mcx_selector` CSR. The CSR may be read
and written in all privilege levels.

.cx_selector_index CSR 0x800
[wavedrom,target="cx_selector_index",svg]
....
{reg:[
{bits: 10, name: 'index'},
...
CX selector table (at the hart's `mcx_selector_table`) to use as the
hart's current CX selector.

In response to CSR-write of `cx_selector_index`, load the 32-bit CX
selector at address (`mcx_selector_table + cx_selector_index.index*4`)
...
the locus moves to a different STATE_ID of a CXU.  `cx_selector_index`
...
The optional CX access control CSRs `mcx_selector_table` and
`cx_selector_index` allow less privileged _user code_ to rapidly
...
address to the hart's `mcx_selector_table` CSR. Then in response to
...
to `cx_selector_index`. In response, the CPU loads from memory (at more
