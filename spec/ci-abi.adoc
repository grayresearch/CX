...
instruction field `req_insn` (<<raw_insn>>) to provide an
...
[TIP]
====
Speculation is more than branch prediction. In a pipelined CPU,
instructions that follow a load or store instruction typically issue
speculatively until the load or store is determined to not raise an
access fault. CF instructions of stateful CFUs must not issue in the
...
* A write `.cs=1` has the side effect of resetting the _current_ state
context to its initial (power up) state.
...
specified by the value of the `rs1` source register. It silently drops
attempts to write state at an invalid state index.
