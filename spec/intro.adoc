== Introduction: a composable composable extension ecosystem
...
[source,C++]
....
#include "ci.h"                         // CX Runtime: class use_ci { ... }
..
use_ci ci(CX_ID_IBitmanip);             // csrrw mcx_selector
uint32_t count = cf(pcnt_cf, data, 0);  // cxu_reg cf_id, rd, rs1, rs2
....
...
....
main() { a(); }
a() { use_ci a_ci(CX_ID_IA); af0; b(); @1 af1; }
b() { use_ci b_ci(CX_ID_IB); bf0; bf1; }
....
...
current composable extension must be `IA` again. Thus the `b_ci()` constructor
