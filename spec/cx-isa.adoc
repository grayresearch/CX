...
* `scxs_index`: CX state context blob index;

* `scxs_data`: CX state context blob data at index.

These mandatory CX CSRs enable user-mode CX software to access a CX state
context's error status, and allow a CX-agnostic runtime or operating
system to manage, initiailze, save, and reload any CX state context.
...
CXU (sans `clk`), if provided, might just have `state_size=0` and/or
`cxs_error=0`. That's fine.
====

==== CX error (`cxs_error`) register
...
[TIP]
====
While `cxs_error` is inessential, it is mandated to provide a *uniform*
...
to express additional aspects of a CX state context.
====

[TIP]
====
A `cxs_error` value of 0 indicates *no error* (stateful CXs) or sometimes
*sorry, no error information* (stateless CXs??).
====

[NOTE]
====
Each CX has a specific behavior contract, including which CX operations
set `cxs_error` and to which values. While there is at present no uniform
specification for `cxs_error` values, we expect to discover and standardize
recommended CX error categories and hence uniform `cxs_error` values.

It remains to be discovered whether `cxs_error` should typically
*accumulate* errors or capture *the last error* (or success)
condition.
====

==== CX state context status (`scxs_status`) register
...
....
{reg:[
{bits: 16, name: 'state_size'},
{bits: 11, name: 'reserved', type:1},
{bits: 2, name: 'cs'},
{bits: 3, name: 0, attr:'version'},
]}
....

==== CX state context index (`scxs_status`) register
...
==== CX state context data (`scxs_data`) register
...
.`scx_guid0` supervisor read-only CX CSR:0x8FF (when MXLEN=32)
[wavedrom,target="`cxs_error`",svg]
....
{reg:[
{bits: 32, name: 'error'},
...
* On system reset, each state context of a stateful extension
...
its state context status automatically changes to `dirty`.
...
to obtain `.state_size`, the size (in XLEN-bit words) of the state context
blob for the selected state context. Allocate array `save[i][]` to store
the serialized state context. For each word in `.state_size`, execute
