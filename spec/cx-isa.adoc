...
supported, each hart has a `scx_table` CSR to address its CX
selector table.
...
shall implement the `scx_table` and `cx_index` CSRs.
...
[TIP]
====
Typically an OS will fill unused `scx_table[]` entries with this invalid
...
==== `scx_table` CSR 0xBC1: CX selector table base

When CX access control (§<<_cx_access_control>>) is supported, the
`MXLEN`-bit-wide `scx_table` CSR specifies the base address
of the hart's CX selector table. The CSR may be read and written in
machine level.

.scx_table CSR 0xBC1 (when MXLEN=32)
...
CSR-writes to `scx_table` zero the twelve least significant
...
CX selector table (at the hart's `scx_table`) to use as the
hart's current CX selector.

In response to CSR-write of `cx_index`, load the 32-bit CX
selector at address (`scx_table + cx_index.index*4`)
...
`cxs_error` is a reserved CX CSR: it may not be used by a CX for any
other purpose. Therefore it may be safely used as a _probe_ after a
selector write, to check whether the selector addresses a valid
CXU and state context:

[source,asm]
....
csrw cx_index,x1        ; select some CXU and state context
csrw cx_status,x0       ; clear cx_status
csrr x0,cxs_error		; probe, discarding error word
...
* The value read must equal the last value read, unless there has been
a CX operation since.
...
virtual machine, operating system, hypervisor) multiplexes software
loci of execution (e.g., request, worker, actor, activity, task, fiber,
continuation, thread, process), upon one or more hardware threads (_harts_).
...
A CXU implementing a stateful composable extension is usually
configured with one state context per hart in the entire system, but
other configurations, including one context per locus, or a small pool
...
mapping of harts to CX state contexts. The resource manager consults
a _System CXU Map_ specifying the mapping CXU_IDs of the configured
composable extensions of the system, and for each CX/CXU, the no. of
state contexts it is configured with. A stateless CXU has zero contexts.
...
for each CX state context of the hart, execute
...
The optional CX access control CSRs `scx_table` and
...
table address to the hart's `scx_table` CSR. Then in response to a system
