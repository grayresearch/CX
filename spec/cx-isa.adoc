...
the current extension (i.e., of the current configured CXU).
...
<<mapping>> defines the mapping from HW-SW interface entities, such as the
`cf_id`, `rd`, `rs1`, `rs2`, `imm`, `csr` fields of the custom function
instruction (or custom CSR access instruction) and the `mcx_selector`
and `cx_status` CSRs, to the CXU Logic Interface's request and response
signals (§<<_cxu_li_signaling>>).
...
|`req_insn` | <- `insn`
|`req_func` | <- `insn.cf_id` {`custom-[012]`} or `csr_cf_id(insn)` {`csrr*`}
...
```
enum { CSRR = 1<<(CF_ID_W-1), CSRRW, CSRRS, CSRRC }; // msb set => CSR access
read_only(insn) = (insn.funct3 == CSRR[SR][I]) && (insn.rs1 == 0);
csr_cf_id(insn) = CSRR + (read_only(insn) ? 0 : insn.funct3[1:0]);
```
